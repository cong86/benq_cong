...
def _extract_pow_state(response: str) -> str | None:
    """Extract pow state from bridge frames like '*POW=OFF#', 'POW=OFF#', or '=OFF#'."""
...
class BenQProjectorCoordinator(DataUpdateCoordinator):
    ...
    def _is_bridge_transient_write_retry(self, operation_name: str) -> bool:
        """Return True for bridge write retries that commonly succeed on reconnect."""
        op = str(operation_name or "").lower()
        return op.startswith("send_command(pow=") or op.startswith(
            "send_raw_fallback(pow="
        ) or op == "send_raw_command"
    ...
    async def _async_retry_operation(
        ...
        if self._is_bridge_transient_write_retry(operation_name):
        ...
        if not await self.projector.connect():
            _LOGGER.warning(
                "%s failed on %s: reconnect retry could not establish session",
                operation_name,
                self.projector.connection,
            )
            return result

        retry_result = await operation()
        if retry_result in (None, False):
            _LOGGER.warning(
                "%s still failed after reconnect retry on %s",
                operation_name,
                self.projector.connection,
            )
