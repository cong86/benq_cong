...
from benqprojector.benqclasses import BenQInvallidResponseError
...
_BRIDGE_BLOCKED_COMMANDS: Final[set[str]] = {
    "macaddr",
    "ltim2",
    "audiosour",
    "micvol",
...
def _patch_bridge_response_parser(projector: BenQProjectorTelnet) -> None:
    ...
    def _parse_response(command, response, lowercase=True):
        ...
        try:
            return _orig_parse(command, response, lowercase)
...
class BenQProjectorConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def validate_input_setup_network(
        ...
        if errors.get(CONF_HOST) is None:
            # Bridge-friendly validation: check TCP reachability only.
            # Full projector probing is deferred to runtime setup to avoid noisy add-flow failures.
