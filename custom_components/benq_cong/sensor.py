...
async def async_setup_entry(
    ...
    entity_descriptions = []
    if coordinator.supports_command("ltim2", "read"):
    ...
    elif coordinator.supports_command("ltim", "read"):
        entity_descriptions.append(
            SensorEntityDescription(
                key="ltim",
    ...
    for entity_description in entity_descriptions:
        if coordinator.supports_command(entity_description.key, "read"):
            entities.append(
...
class BenQProjectorSensor(CoordinatorEntity, SensorEntity):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if self.coordinator.power_status in [
            BenQProjector.POWERSTATUS_UNKNOWN,
            BenQProjector.POWERSTATUS_POWERINGOFF,
        ]:
            self._attr_available = False
